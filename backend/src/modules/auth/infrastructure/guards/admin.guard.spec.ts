...
describe('AdminGuard', () => {
	...
	it('should throw ForbiddenException for moderator users', async () => {
		await expect(guard.canActivate(makeContext('moderator'))).rejects.toThrow(ForbiddenException);
	});
	...
	it('should throw ForbiddenException for store organisers', async () => {
		await expect(guard.canActivate(makeContext('user', 'store_organiser'))).rejects.toThrow(
			ForbiddenException,
		);
	});
});
