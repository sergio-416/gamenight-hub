...
describe('StoreOrganiserGuard', () => {
	...
	it('should allow store organisers', async () => {
		await expect(guard.canActivate(makeContext('user', 'store_organiser'))).resolves.toBe(true);
	});

	it('should allow admins regardless of userType', async () => {
		await expect(guard.canActivate(makeContext('admin', 'regular'))).resolves.toBe(true);
	});

	it('should throw ForbiddenException for regular users', async () => {
		await expect(guard.canActivate(makeContext('user', 'regular'))).rejects.toThrow(
			ForbiddenException,
		);
	});

	it('should throw ForbiddenException for moderators', async () => {
		await expect(guard.canActivate(makeContext('moderator', 'regular'))).rejects.toThrow(
			ForbiddenException,
		);
	});
});
