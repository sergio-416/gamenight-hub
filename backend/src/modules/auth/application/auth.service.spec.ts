...
describe('AuthService', () => {
	...
	describe('generateMagicLink', () => {
		it('should generate link with correct ActionCodeSettings', async () => {
			mockGenerateSignInWithEmailLink.mockResolvedValue('https://magic.link/token');

			await service.generateMagicLink('user@example.com');

			expect(mockGenerateSignInWithEmailLink).toHaveBeenCalledWith('user@example.com', {
		...
		it('should throw on Firebase Admin failure', async () => {
			mockGenerateSignInWithEmailLink.mockRejectedValue(new Error('Firebase Admin error'));

			await expect(service.generateMagicLink('user@example.com')).rejects.toThrow(
