...
export class XpService {
      ...
      this.#db
        ...
        .where(
          and(
            ...
            gte(xpTransactions.createdAt, monthStart)
          )
        ),
      this.#db
        .select({
  ...
  async awardXp(
    ...
    return this.#db.transaction(async (tx) => {
      ...
      const cappedBase = this.#calculator.applyDailyCap(
        baseXp,
        dailyActionSpent,
        dailyGrandSpent
      );

      const finalXp = this.#calculator.calculateFinalXp(
        cappedBase,
        profile.streakWeeks,
        now
      );

      const multiplier = this.#calculator.getCombinedMultiplier(
        profile.streakWeeks,
        now
      );
      ...
      const newMonthlyGameAdds =
        action === "game_added" ? monthlyGameAdds + 1 : monthlyGameAdds;
      ...
      await tx
        ...
        .set({
          ...
          monthlyGameAddsResetAt:
            monthlyGameAdds === 0 && newMonthlyGameAdds > 0
              ? now
              : profile.monthlyGameAddsResetAt,
          updatedAt: now,
        })
        .where(eq(xpProfiles.userId, userId));
