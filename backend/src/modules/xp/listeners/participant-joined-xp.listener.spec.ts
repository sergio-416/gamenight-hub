...
describe('ParticipantJoinedXpListener', () => {
	...
	beforeEach(async () => {
		...
		const module: TestingModule = await Test.createTestingModule({
			providers: [ParticipantJoinedXpListener, { provide: XpService, useValue: mockXpService }],
		}).compile();

		listener = module.get<ParticipantJoinedXpListener>(ParticipantJoinedXpListener);
	});
	...
	it('should call awardXp with correct params', async () => {
		...
		expect(mockXpService.awardXp).toHaveBeenCalledWith('user-1', 'participant_joined', {
			eventId: 'event-1',
		});
	});
