...
describe('LocationsController', () => {
	...
	describe('POST /locations', () => {
		it('should create a new game night location when valid data provided', async () => {
			...
			expect(mockLocationsService.create).toHaveBeenCalledWith(createDto, MOCK_UID);
		});
	});
	...
	describe('PATCH /locations/:id', () => {
		it('should update location when valid data provided', async () => {
			...
			expect(mockLocationsService.update).toHaveBeenCalledWith('uuid-123', updateDto, MOCK_UID);
		});
	});
	...
	describe('DELETE /locations/:id', () => {
		it('should remove location when valid id provided', async () => {
			...
			expect(result.id).toBe('uuid-123');
			expect(mockLocationsService.remove).toHaveBeenCalledWith('uuid-123', MOCK_UID);
		});
	});
