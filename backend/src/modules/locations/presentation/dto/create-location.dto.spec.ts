...
describe('CreateLocationSchema', () => {
	...
	describe('when validation fails', () => {
		...
		it('should reject location without longitude', () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues.some((i) => i.path.includes('longitude'))).toBe(true);
			}
		});
		...
		it('should reject location with invalid venueType', () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues.some((i) => i.path.includes('venueType'))).toBe(true);
			}
		});
...
describe('CreateLocationWithEventSchema', () => {
	...
	it('should not carry eventDate on the base CreateLocationSchema', () => {
		...
		expect(result.success).toBe(true);
		if (result.success) {
			expect((result.data as Record<string, unknown>).eventDate).toBeUndefined();
		}
	});
});
