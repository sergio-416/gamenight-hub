...
describe('LocationsService', () => {
	...
	describe('create location', () => {
		it('should persist new game night location to database', async () => {
			...
			expect(mockEventEmitter.emit).toHaveBeenCalledWith('location.created', expect.anything());
		});
	});

	describe('find all locations', () => {
		it('should return all persisted locations from database', async () => {
			const mockLocations = [makeLocation(), makeLocation({ id: 'uuid-2', name: 'Store' })];
			mockDb.select
	...
	describe('update location', () => {
		...
		it('should throw ForbiddenException when updating location owned by another user', async () => {
			...
			await expect(service.update(location.id, { name: 'Hijacked' }, OWNER_UID)).rejects.toThrow(
				ForbiddenException,
			);
		});
	});
	...
	describe('remove location', () => {
		...
		it('should throw ForbiddenException when deleting location owned by another user', async () => {
			...
			await expect(service.remove(location.id, OWNER_UID)).rejects.toThrow(ForbiddenException);
		});
	});
