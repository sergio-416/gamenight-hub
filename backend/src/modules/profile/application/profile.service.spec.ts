...
describe("ProfileService", () => {
  ...
  describe("updateMyProfile", () => {
    ...
    it("should throw ConflictException when username is already taken by another user", async () => {
      ...
      mockDb.select
        ...
        .mockReturnValueOnce(
          chainResolving([
            makeProfile({ uid: OTHER_UID, username: "taken_name" }),
          ])
        );

      await expect(
        service.updateMyProfile(OWNER_UID, { username: "taken_name" })
  ...
  describe("getPublicProfile", () => {
    ...
    it("should throw NotFoundException when username does not exist", async () => {
      ...
      await expect(service.getPublicProfile("ghost_user")).rejects.toThrow(
        NotFoundException
      );
    });
    ...
    it("should throw NotFoundException when profile exists but is private", async () => {
      ...
      await expect(service.getPublicProfile("john_doe")).rejects.toThrow(
        NotFoundException
      );
    });
  });
  ...
  describe("deleteMyProfile", () => {
    it("should successfully soft delete profile and clear all PII fields", async () => {
      ...
      mockDb.update.mockReturnValue(
        chainResolving([{ ...profile, deletedAt: new Date() }])
      );
    ...
    it("should throw NotFoundException when profile does not exist", async () => {
      ...
      await expect(service.deleteMyProfile(OWNER_UID)).rejects.toThrow(
        NotFoundException
      );
    });
