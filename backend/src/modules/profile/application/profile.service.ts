...
export class ProfileService {
      ...
      if (isActuallyChangingName && nameIsAlreadySet) {
        if (current.nameChangedAt !== null) {
          const daysSinceLastChange =
            (Date.now() - new Date(current.nameChangedAt).getTime()) /
            (1000 * 60 * 60 * 24);

          if (daysSinceLastChange < NAME_CHANGE_COOLDOWN_DAYS) {
            const daysRemaining = Math.ceil(
              NAME_CHANGE_COOLDOWN_DAYS - daysSinceLastChange
            );
  ...
  async deleteMyProfile(
    ...
    const { eligible, openEventsCount } =
      await this.getDeletionEligibility(uid);
    ...
    try {
      ...
      await this.#db
        .update(locations)
        .set({ deletedAt: now })
        .where(eq(locations.createdBy, uid));
      ...
      this.#logger.error(
        `[deleteMyProfile] DB cascade failed: ${String(dbErr)}`
      );
      throw new InternalServerErrorException({
