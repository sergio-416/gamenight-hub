...
describe('ProfileController', () => {
	...
	describe('GET /profile/me', () => {
		it("should return the authenticated user's own profile", async () => {
			...
			expect(mockProfileService.getMyProfile).toHaveBeenCalledWith(MOCK_UID, MOCK_EMAIL);
		});
		...
		it('should throw NotFoundException when profile has been soft-deleted', async () => {
			...
			await expect(controller.getMyProfile(MOCK_UID, MOCK_EMAIL)).rejects.toThrow(
				NotFoundException,
			);
		});
	});
	...
	describe('PATCH /profile/me', () => {
		it('should update and return the profile with provided fields', async () => {
			...
			expect(mockProfileService.updateMyProfile).toHaveBeenCalledWith(MOCK_UID, {
				bio: 'Board game fanatic',
				location: 'Barcelona',
			});
		});
	...
	describe('GET /profile/:username', () => {
		it('should return sanitised public profile when username exists and profile is public', async () => {
			...
			expect(mockProfileService.getPublicProfile).toHaveBeenCalledWith('john_doe');
		});
	});
	...
	describe('GET /profile/:username/games', () => {
		it('should return paginated games when profile is public and collection is visible', async () => {
			...
			const result = await controller.getPublicGames('john_doe', PAGINATION.DEFAULT_PAGE, PAGINATION.PUBLIC_COLLECTION_LIMIT);
		...
		it('should throw NotFoundException when collection is hidden', async () => {
			...
			await expect(controller.getPublicGames('john_doe', PAGINATION.DEFAULT_PAGE, PAGINATION.PUBLIC_COLLECTION_LIMIT)).rejects.toThrow(NotFoundException);
		});
	});
