...
describe("UpdatePersonalFieldsSchema", () => {
  ...
  describe("when validation fails", () => {
    it("should reject status field that is not a valid status value", () => {
      ...
      if (!result.success) {
        expect(result.error.issues.some((i) => i.path.includes("status"))).toBe(
          true
        );
      }
    });
    ...
    it("should reject notes field that is not a string", () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues.some((i) => i.path.includes("notes"))).toBe(
          true
        );
      }
    });
    ...
    it("should reject complexity less than 1", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.path.includes("complexity"))
        ).toBe(true);
      }
    });
