...
describe('GamesCrudService', () => {
	...
	describe('findOne', () => {
		...
		it('should throw NotFoundException when game belongs to a different user', async () => {
			...
			await expect(service.findOne('some-id', OWNER_UID)).rejects.toThrow(NotFoundException);
		});

		it('should throw NotFoundException when game does not exist', async () => {
			mockDb.select.mockReturnValue(chainResolving([]));

			await expect(service.findOne('nonexistent-id', OWNER_UID)).rejects.toThrow(NotFoundException);
		});
	});

	describe('findAll', () => {
		it('should return paginated results scoped to the requesting user', async () => {
			const mockGames = [makeGame(), makeGame({ id: 'uuid-2', name: 'Ticket to Ride' })];
			mockDb.select
	...
	describe('update', () => {
		...
		it('should throw NotFoundException when updating a game owned by another user', async () => {
			...
			await expect(
				service.update('some-other-id', { status: 'played' as const }, OWNER_UID),
	...
	describe('remove', () => {
		...
		it('should throw NotFoundException when removing a game owned by another user', async () => {
			...
			await expect(service.remove('some-other-id', OWNER_UID)).rejects.toThrow(NotFoundException);
		});
	});
	...
	describe('findOneEnriched', () => {
		it('should return game with bggRating and bggRank from LEFT JOIN', async () => {
			const game = makeGame();
			mockDb.select.mockReturnValue(chainResolving([{ game, bggRating: '8.30', bggRank: 16 }]));
		...
		it('should return null rating/rank when no bgg_games match', async () => {
			...
			mockDb.select.mockReturnValue(chainResolving([{ game, bggRating: null, bggRank: null }]));
		...
		it('should throw NotFoundException when game not found', async () => {
			...
			await expect(service.findOneEnriched('nonexistent', OWNER_UID)).rejects.toThrow(
				NotFoundException,
			);
		});
	});
