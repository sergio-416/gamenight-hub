...
describe('BggCsvService', () => {
	...
	it('should handle fuzzy matching with typos', () => {
		...
		expect(results.length).toBeGreaterThan(0);
		expect(results.some((r) => r.name.toLowerCase().includes('catan'))).toBe(true);
	}, 15_000);
	...
	it('should match ignoring punctuation', () => {
		...
		expect(results.length).toBeGreaterThan(0);
		expect(results.some((r) => r.name.toLowerCase().includes('ticket'))).toBe(true);
	}, 15_000);
	...
	it('should match ignoring articles', () => {
		...
		expect(results.length).toBeGreaterThan(0);
		expect(results.some((r) => r.name.toLowerCase().includes('castles'))).toBe(true);
	}, 15_000);
	...
	it('should match joined game names written with spaces', () => {
		...
		expect(results.length).toBeGreaterThan(0);
		expect(results.some((r) => r.name.toLowerCase().includes('micromacro'))).toBe(true);
	}, 15_000);
	...
	it('should boost ranked games above unranked', () => {
		...
		const firstRanked = results.findIndex((r) => r.rank !== undefined && r.rank !== 'Not Ranked');
