...
import { games, type SelectUserPlayedGame, userPlayedGames } from '@database/schema/index.js';
...
export class UserPlayedGamesService {
	...
	async markAsPlayed(userId: string, gameId: string): Promise<SelectUserPlayedGame> {
		...
		const [existing] = await this.db
			...
			.where(and(eq(userPlayedGames.userId, userId), eq(userPlayedGames.gameId, gameId)))
			.limit(1);
		...
		await this.db.delete(games).where(and(eq(games.id, gameId), eq(games.createdBy, userId)));
