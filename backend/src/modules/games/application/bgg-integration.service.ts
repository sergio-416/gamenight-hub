...
type BggItem = {
	$: { id: string };
	name?: Array<{ $: { value: string; type?: string } }> | { $: { value: string; type?: string } };
...
export class BggIntegrationService {
	...
	async #enforceRateLimit(): Promise<void> {
		...
		if (timeSinceLastRequest < this.#rateLimitDelay) {
			...
			this.#logger.debug(`Rate limiting: waiting ${waitTime}ms before next BGG request`);
	...
	async getGameDetails(bggId: number): Promise<BggGameDetails> {
		...
		try {
			...
			const result: BggGameDetails = {
				...
				minAge: item.minage?.$.value ? parseInt(item.minage.$.value, 10) : undefined,
			...
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			this.#logger.error(`Failed to fetch BGG game details for ${bggId}: ${errorMessage}`);
