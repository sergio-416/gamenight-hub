...
describe('EventsController', () => {
	...
	describe('PATCH /events/:id', () => {
		it('should update event when valid data provided', async () => {
			...
			expect(mockEventsService.update).toHaveBeenCalledWith('uuid-123', updateDto, MOCK_UID);
		});
	});
	...
	describe('DELETE /events/:id', () => {
		it('should remove event when valid id provided', async () => {
			...
			expect(result.id).toBe('uuid-123');
			expect(mockEventsService.remove).toHaveBeenCalledWith('uuid-123', MOCK_UID);
		});
	});
});
