...
export class Stats {
	...
	readonly statsResource = httpResource<StatsData>(() =>
		this.isLoggedIn() && this.view() === "games"
			? `${this.#apiUrl}/games/stats`
			: undefined,
	);
	...
	readonly adminStatsResource = httpResource<AdminStatsDto>(() =>
		this.isLoggedIn() && this.view() === "admin"
			? `${this.#apiUrl}/stats/admin`
			: undefined,
	);

	readonly gamesByCategoryData = computed(
		() => this.statsResource.value()?.gamesByCategory ?? [],
	);
	...
	readonly totalGames = computed(
		() => this.statsResource.value()?.totalGames ?? 0,
	);
	...
	readonly gamesByCategoryOptions = computed<ChartOptions>(() => ({
		...
		title: {
			text: this.#transloco.translate(
				"stats.collection.charts.gamesByCategory",
			),
		},
	...
	readonly collectionGrowthOptions = computed<ChartOptions>(() => ({
		...
		title: {
			text: this.#transloco.translate(
				"stats.collection.charts.collectionGrowth",
			),
		},
		xaxis: { type: "datetime" },
	}));
	...
	readonly complexityDistributionOptions = computed<ChartOptions>(() => ({
		...
		title: {
			text: this.#transloco.translate(
				"stats.collection.charts.complexityDistribution",
			),
		},
