...
function createMockAuthOps(onAuthImpl?: AuthStateCallback) {
	return {
		onAuthStateChanged: vi
			...
			.mockImplementation(
				onAuthImpl ??
					((_auth: unknown, _cb: (user: unknown) => void) => () => {}),
			),
		...
		signInWithEmailLink: vi
			.fn()
			.mockResolvedValue({ user: null, operationType: "signIn" }),
...
describe("AuthService", () => {
	...
	describe("login", () => {
		...
		it("should update currentUser signal when auth state fires after login", async () => {
			...
			await vi.waitFor(() =>
				expect(mockNotifications.connect).toHaveBeenCalledWith(
					"mock-token",
					"user-123",
				),
			);
	...
	describe("isLoggedIn", () => {
		it("should return true when user is logged in", async () => {
			...
			authStateCallback(mockUser);
			await vi.waitFor(() =>
				expect(mockNotifications.connect).toHaveBeenCalled(),
			);
	...
	describe("userRole signal", () => {
		it("should be 'user' when no user is logged in", () => {
			const { service } = configureTestModule(
				(_: unknown, cb: (user: unknown) => void) => {
					cb(null);
					return () => {};
				},
			);
	...
	describe("userType signal", () => {
		...
		it("should be 'store_organiser' for organiser accounts", async () => {
			...
			await vi.waitFor(() =>
				expect(service.userType()).toBe("store_organiser"),
			);
		});
	...
	describe("completeSignInWithLink", () => {
		...
		it("should return isNewUser true when signing in for the first time", async () => {
			...
			const result = await service.completeSignInWithLink(
				"user@example.com",
				validLink,
			);
		...
		it("should throw when the link is not a valid sign-in link", async () => {
			...
			await expect(
				service.completeSignInWithLink(
					"user@example.com",
...
describe("translateAuthError", () => {
	...
	it("should return a message for invalid email errors", () => {
		expect(translateAuthError(new Error("auth/invalid-email"))).toMatch(
			/valid email/i,
		);
	});

	it("should return a message for network errors", () => {
		expect(translateAuthError(new Error("network-request-failed"))).toMatch(
			/network/i,
		);
	});
	...
	it("should return a message for expired sign-in link errors", () => {
		expect(translateAuthError(new Error("auth/expired-action-code"))).toMatch(
			/expired/i,
		);
	});

	it("should return a message for already-used sign-in link errors", () => {
		expect(translateAuthError(new Error("auth/invalid-action-code"))).toMatch(
			/already been used/i,
		);
	});

	it("should return a fallback message for unknown errors", () => {
		expect(translateAuthError(new Error("something-random"))).toMatch(
			/try again/i,
		);
	});
