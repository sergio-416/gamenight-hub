...
export const handlers = [
	http.get(`${BASE_URL}/games`, () => HttpResponse.json(paginateData(mockGames))),
	...
	http.get(`${BASE_URL}/games/:id`, ({ params }) =>
		HttpResponse.json(mockGames.find((g) => g.id === params['id']) ?? mockGames[0]),
	),
	http.post(`${BASE_URL}/games/import/:bggId`, () => HttpResponse.json(mockGames[0])),
	http.delete(`${BASE_URL}/games/:id`, ({ params }) =>
		HttpResponse.json(mockGames.find((g) => g.id === params['id']) ?? mockGames[0]),
	),
	http.get(`${BASE_URL}/locations`, () => HttpResponse.json(paginateData(mockLocations))),
	http.get(`${BASE_URL}/locations/bounds`, () => HttpResponse.json(mockLocations)),
	...
	http.delete(`${BASE_URL}/locations/:id`, ({ params }) =>
		HttpResponse.json(mockLocations.find((l) => l.id === params['id']) ?? mockLocations[0]),
	),
	http.get(`${BASE_URL}/events`, () => HttpResponse.json(paginateData(mockEvents))),
	...
	http.delete(`${BASE_URL}/events/:id`, ({ params }) =>
		HttpResponse.json(mockEvents.find((e) => e.id === params['id']) ?? mockEvents[0]),
	),
];
